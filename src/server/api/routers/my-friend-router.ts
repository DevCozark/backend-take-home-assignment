  ...
  getById: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return ctx.db.connection().execute(async (conn) => {
         ...
         */
        const userMutualFriendCountResult = await userMutualFriendCount(
          conn,
          ctx.session.userId,
          input.friendUserId
        )
        return (
          conn
            ...
            .then((res) => {
              return { ...res, mutualFriendCount: userMutualFriendCountResult }
            })
        )
      })
    }),
...
const userMutualFriendCount = async (
  db: Database,
  userId: number,
  friendUserId: number
) => {
  const res = await db
    .selectFrom('friendships as friendsA')
    .innerJoin(
      'friendships as friendsB',
      'friendsB.friendUserId',
      'friendsA.friendUserId'
    )
    .where('friendsA.userId', '=', userId)
    .where('friendsB.userId', '=', friendUserId)
    .where('friendsA.status', '=', FriendshipStatusSchema.Values['accepted'])
    .select((eb) => [
      'friendsA.userId as userId',
      eb.fn.count('friendsA.friendUserId').as('totalMutualFriendCount'),
    ])
    .groupBy('friendsA.userId')
    .executeTakeFirst()
  return res?.totalMutualFriendCount || 0
}
